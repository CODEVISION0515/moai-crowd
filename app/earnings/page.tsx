    ...
    <div className="mx-auto max-w-4xl px-4 py-10">
          ...
          <tbody>
            {contracts?.map((c: any) => (
              <tr key={c.id} className="border-t border-slate-200">
                ...
                <td className="p-3 text-right">¥{c.worker_payout_jpy.toLocaleString()}</td>
                <td className="p-3 text-right text-red-600">- ¥{(c.withholding_tax_jpy ?? 0).toLocaleString()}</td>
                <td className="p-3 text-right font-semibold">¥{(c.net_payout_jpy ?? c.worker_payout_jpy).toLocaleString()}</td>
              </tr>
            ))}
      ...
      <div className="card mt-6 bg-amber-50 border-amber-200 text-sm">
        ...
        <ul className="space-y-1 text-slate-700">
          <li>• 個人事業主の方は、総報酬 ¥{payout.toLocaleString()} を事業所得として計上</li>
          <li>• 源泉徴収額 ¥{withheld.toLocaleString()} は前払税額として確定申告で精算</li>
          <li>• プラットフォーム手数料 ¥{fee.toLocaleString()} は経費として計上可能</li>
